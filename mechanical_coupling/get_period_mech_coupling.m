function [ T, cycle, tees ] =get_period_mech_coupling( tau_f, c_MA, tau_m )
...
for ii=round((3/4)*size(t,1)):size(t,1)
   ...
   else if abs(y(ii,1)) < tol && start_flag == 1 && y(ii,1)>y(ii-1,1) && upward_flag == 1
           end_time = t(ii);
           end_ind = ii;
           break
       else if abs(y(ii,1)) < tol && start_flag == 1 && y(ii,1)<y(ii-1,1) && upward_flag == 0
            end_time = t(ii);
            end_ind = ii;
            break
           end
