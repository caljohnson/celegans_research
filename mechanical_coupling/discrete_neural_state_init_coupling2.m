...
function [ updater_handle ] = discrete_neural_state_init_coupling2(K_init, off_thresh, on_thresh, is_V)
%DISCRETE_NEURAL_STATE_INIT_COUPLING2 Initializes the discrete neural state updater
...
%compute neural state and update
    function state = discrete_neural_state_updater(K1,K2)
        if( (100*K1+0*K2)/100 > on_thresh ) %if pass upper threshold, turn on
            state = 1;
            S = state;
        elseif( (100*K1+0*K2)/100 < off_thresh ) %if pass lower threshold, turn off
            state = 0;
            S = state;
        else
            state = S;
